...
var products1 = document.querySelector(".acceHolder");

function creatCard(productList) {
    for (let i = 0; i < productList.length; i++) {
        if (productList[i].isAccessory == false) {
            function code() {
                ...
                title2.innerText = productList[i].brand;

                const title3 = document.createElement("h5");
                title3.innerText = productList[i].price;

                // appending the children
                Cardinfo.appendChild(title);
                Cardinfo.appendChild(title2);
                Cardinfo.appendChild(title3);
